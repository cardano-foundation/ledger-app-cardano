...
#define TESTCASE(path_, expected_) \
	{ \
		...
		derivePrivateKey(path_, ARRAY_LEN(path_), &chainCode, &privateKey); \
		stream_t s; \
		stream_init(&s); \
...
#define TESTCASE(path_, expected_) \
	{ \
		...
		extractRawPublicKey(&publicKey, publicKeyRaw); \
		stream_t s; \
		stream_init(&s); \
		stream_appendFromHexString(&s, expected_); \
		const uint8_t* expectedBuffer = stream_head(&s); \
