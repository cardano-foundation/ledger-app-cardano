...
#define TESTCASE(outputIndex, expectedAmount) \
	{ \
		...
		for (unsigned i =0; i < ARRAY_LEN(txChunks); i++) { \
			stream_appendFromHexString(& state.stream, (const char*) PIC(txChunks[i])); \
			BEGIN_TRY { \
				TRY { \
					keepParsing(&state); \
				} \
...
{
	TESTCASE(0, 1673668090925);
	TESTCASE(1,     372500000);
	TESTCASE(2,     433000500);
	TESTCASE(3,    3280715000);
	TESTCASE(4,    2035261700);
	TESTCASE(5, LOVELACE_INVALID);
}
